...
const WhatsAppFloat = () => {
  ...
  const [prefill, setPrefill] = useState("Olá! Gostaria de saber mais sobre as viagens da SL Turismo.");
  ...
  useEffect(() => {
    supabase.from("site_content").select("content").eq("section_key", "footer").maybeSingle().then(({ data }) => {
      if (data?.content) {
        ...
        const digits = (c.phone_link || "").replace(/\D/g, "");
        if (digits) setPhoneLink(digits);
        if (typeof c.whatsapp_prefill === "string" && c.whatsapp_prefill.trim()) setPrefill(c.whatsapp_prefill.trim());
      }
    });
  }, []);
  ...
  return (
    <a
      href={`https://wa.me/${digits}?text=${encodeURIComponent(prefill)}`}
      target="_blank"
